...
export const DocPagination: React.FC<DocPaginationProps> = ({
  ...
  return (
    <nav aria-label="Documentation Pagination" className="pt-8 mt-12 border-t border-[#181818] flex items-center justify-between gap-4 select-none">
      {prevDoc ? (
        <button
          onClick={() => onNavigateTopic(prevDoc.id)}
          className="group inline-flex items-center gap-2 text-sm text-[#888888] hover:text-white transition-colors py-1 text-left"
        >
          <ArrowLeft className="w-4 h-4 text-[#666666] group-hover:text-white transition-transform group-hover:-translate-x-1" />
      ...
      {nextDoc && (
        <button
          ...
          className="group inline-flex items-center gap-2 text-sm text-[#888888] hover:text-white transition-colors py-1 ml-auto text-right"
        >
          <span className="font-medium">{nextDoc.title}</span>
          <ArrowRight className="w-4 h-4 text-[#666666] group-hover:text-white transition-transform group-hover:translate-x-1" />
        </button>
      )}
    </nav>
  );
